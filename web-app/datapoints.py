...
def engineer_features(DataFrame):
    ...
    self.Dataframe['num_ticket_types'] = [len(i) for i inself.Dataframe['ticket_types']]
    self.Dataframe['total_prev_payouts'] = [len(i) for i inself.Dataframe['previous_payouts']]
    self.Dataframe["payout_specified"] = [0 if method == '' else 1 for method inself.Dataframe["payout_type"]]
    self.Dataframe["channel_is_0"] = [1 if channel == 0 else 0 for channel inself.Dataframe["channels"]]
    self.Dataframe["channel_is_5"] = [1 if channel == 5 else 0 for channel inself.Dataframe["channels"]]
    self.Dataframe["channel_is_6"] = [1 if channel == 6 else 0 for channel inself.Dataframe["channels"]]
    self.Dataframe["delivery_method_0"] = [1 if method == 0 else 0 for method inself.Dataframe["delivery_method"]]
    self.Dataframe.fillna(0, inplace=True)
...
def total_quantity_tickets(DataFrame):
    totals = [] 
    for row inself.Dataframe['ticket_types']:
        row_totals = [] 
        for i in row: 
...
def drop_cols(DataFrame):
    self.Dataframe.drop(['acct_type', 'approx_payout_date', 'channels', 'country',
